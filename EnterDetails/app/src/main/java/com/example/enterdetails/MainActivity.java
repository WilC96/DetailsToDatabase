...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener, DatePickerDialog.OnDateSetListener {
    ...
    TextInputEditText txtNameTxt, txtEmailTxt;
    ...
    SharedPreferences.Editor appPrefEdit;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /*
        // save instance - works for variables not text in text-view via keyboard input
    ...
    public void initializeUI() {
        ...
        // TextInputEditText type required to .setText via java
        txtNameTxt = findViewById(R.id.txtNameText);
        txtEmailTxt = findViewById(R.id.txtEmailText);

        // save data into preferences
        appPref = PreferenceManager.getDefaultSharedPreferences(this);
        rememberMeChk = findViewById(R.id.chkRemember);

        name = appPref.getString("keyName", "");
        email = appPref.getString("keyEmail", "");

        toRemember = appPref.getBoolean("bool", false);
        txtNameTxt.setText(name);
        txtEmailTxt.setText(email);
        rememberMeChk.setChecked(toRemember);
    }
    ...
    public void openDisplay() {
        ...
        assignTextInputLayoutToString();

        if(rememberMeChk.isChecked()){

            appPrefEdit=appPref.edit();
            appPrefEdit.putString("keyName", name);
            appPrefEdit.putString("keyEmail", email);
            appPrefEdit.putBoolean("bool", true);
            appPrefEdit.commit();

        }else{
            appPrefEdit=appPref.edit();
            // for singular removal: editor.remove("KeyEmail");
            appPrefEdit.clear();
            appPrefEdit.commit();
        }
    ...
    public void assignTextInputLayoutToString() {
